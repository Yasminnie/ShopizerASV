...
public class ShoppingOrderController extends AbstractController {
    ...
    private static final String REDIRECT_SHOPPINGCART = "redirect:/shop/cart/shoppingCart.html";
    private static final String MODULE_SHIPPING = "module.shipping.";
    private static final String NOTE_STRING = ".note";

    @SuppressWarnings("unused")
    @RequestMapping(value = "/checkout.html", method = GET)
            ...
            if (StringUtils.isBlank(shippingReturnCode) || shippingReturnCode.equals(ShippingQuote.NO_POSTAL_CODE)) {
                ...
                if (!CollectionUtils.isEmpty(options)) {
                    ...
                    for (ShippingOption shipOption : options) {
                        ...
                        String note = messages.getMessage(moduleName.append(NOTE_STRING).toString(), locale, "");
                        ...
                        if (!StringUtils.isBlank(shipOption.getOptionCode())) {
                            ...
                            try {
                                ...
                                optionCodeBuilder.append(MODULE_SHIPPING).append(shipOption.getShippingModuleCode());
    ...
    @RequestMapping(value = "/commitPreAuthorized.html", method = GET)
    ...
    private Order commitOrder(ShopOrder order, HttpServletRequest request, Locale locale) throws Exception {
        ...
        setUsernameAndPassword(customer, store, request, auth);
    ...
    public String commitOrder(@CookieValue("cart") String cookie, @Valid @ModelAttribute(value = "order")
        ...
        if (configs.containsKey(Constants.DEBUG_MODE)) {
            Boolean debugMode = (Boolean) configs.get(Constants.DEBUG_MODE);
            if (debugMode) {
                try {
                    ObjectMapper mapper = new ObjectMapper();
                    ...
                    for (ShippingOption shipOption : options) {
                        ...
                        if (!StringUtils.isBlank(shipOption.getOptionCode())) {
                            ...
                            try {
                                ...
                                optionCodeBuilder.append(MODULE_SHIPPING).append(shipOption.getShippingModuleCode()).append(".").append(shipOption.getOptionCode());
                                String optionName = messages.getMessage(optionCodeBuilder.toString(), locale);
                                shipOption.setOptionName(optionName);
                            } catch (Exception e) {//label not found
                                LOGGER.warn("No shipping code found for " + optionCodeBuilder.toString());
